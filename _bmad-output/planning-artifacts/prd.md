...
- **B-43: Fix app2 domain — move from `app2.lenie-ai.eu` to `app2.dev.lenie-ai.eu`** — CloudFront distribution `E1NHFTM571WQ7L` currently has alias `app2.lenie-ai.eu` and Route53 A record points to it at `app2.lenie-ai.eu`. Should be `app2.dev.lenie-ai.eu` to match the `dev` environment convention (`api.dev.lenie-ai.eu`, `app.dev.lenie-ai.eu`, `www.lenie-ai.eu`). Steps: (1) Update CloudFront alternate domain name to `app2.dev.lenie-ai.eu`, (2) Update `cloudfront-app2.yaml` template with correct alias, (3) Update Route53 A record from `app2.lenie-ai.eu` → `app2.dev.lenie-ai.eu`, (4) Remove old `app2.lenie-ai.eu` A record, (5) Update `deploy.sh` echo messages, (6) Verify ACM certificate covers `app2.dev.lenie-ai.eu` (wildcard `*.dev.lenie-ai.eu` or add SAN).
