...
**Secrets Management (NFR16-NFR17)**

- NFR16: No secrets (API keys, passwords, tokens) remain in `.env` files after migration — only bootstrap variables
- NFR17: Backend starts successfully in both Docker (Vault) and AWS (SSM/Secrets Manager) environments using the unified config loader

**B-63: Migrate .env Configuration to Secret Managers (FR33-FR39)**

- FR33: Create unified config loader module in `backend/library/` supporting Vault and AWS SSM/Secrets Manager
- FR34: Bootstrap `.env` variables: `SECRETS_BACKEND`, `VAULT_ADDR`, `AWS_REGION`, `ENV_DATA`
- FR35: Store and load all config from HashiCorp Vault for Docker/NAS environment
- FR36: Store and load all config from AWS SSM Parameter Store / Secrets Manager for AWS environment
- FR37: Update `server.py` to use config loader instead of `os.environ`
- FR38: Update Lambda handlers to use config loader instead of environment variables
- FR39: Update `.env_example` to bootstrap-only variables with documentation
...
FR33: Epic 20 — Create unified config loader module
FR34: Epic 20 — Bootstrap .env variables (SECRETS_BACKEND, VAULT_ADDR, AWS_REGION)
FR35: Epic 20 — Store config in HashiCorp Vault (Docker/NAS)
FR36: Epic 20 — Store config in AWS SSM/Secrets Manager
FR37: Epic 20 — Update server.py to use config loader
FR38: Epic 20 — Update Lambda handlers to use config loader
FR39: Epic 20 — Update .env_example to bootstrap-only
...
Implementation notes:
- B-63: Core security improvement — eliminates secrets from `.env` files
- Story 20-1: Unified config loader abstraction (Vault + AWS backends)
- Story 20-2: Vault backend implementation + Docker/NAS integration
- Story 20-3: AWS SSM/Secrets Manager backend implementation
- Story 20-4: Integrate config loader into server.py and Lambda handlers
- Story 20-5: Update .env_example and document new config flow
- Execution order: 20-1 → 20-2 + 20-3 (parallel) → 20-4 → 20-5
- Vault is already installed on NAS — no infrastructure provisioning needed
- AWS SSM/Secrets Manager already partially used (deploy scripts use SSM for bucket names)
