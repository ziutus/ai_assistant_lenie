...
type Form = Pick<Feed, "type" | "url" | "channel_id" | "language" | "tags" | "auto_import" | "auto_import_after" | "disabled" | "default_state">;
...
export default function Feeds() {
  ...
  const load = React.useCallback(async () => {
    const response = await fetch(`${apiUrl}/feed_sources`, { headers });
    const data = await readJson(response);
    if (!response.ok) throw new Error(data.message || "Nie udało się pobrać feedów");
  ...
  const save = async (feed: Feed) => {
    ...
    try {
      const payload = { ...form, url: form.type === "youtube_channel" ? null : (form.url || "").trim(), channel_id: form.type === "youtube_channel" ? form.channel_id?.trim() : null, auto_import_after: toIso(form.auto_import_after || "") };
