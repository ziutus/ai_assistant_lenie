...
describe("SearchCriteriaEditor", () => {
  it("edits topic and filter chips", () => {
    ...
    expect(onChange.mock.calls.at(-1)?.[0].query).toBe("energia");
    fireEvent.change(screen.getByLabelText("Okres od"), { target: { value: "2010" } });
    expect(onChange.mock.calls.at(-1)?.[0].subject_period_start_year).toBe(2010);
  });
  ...
  it("removes a filter and applies corrected criteria", () => {
    ...
    expect(onChange.mock.calls.at(-1)?.[0].publisher_name).toBeNull();
