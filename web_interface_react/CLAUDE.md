...
| `/search` | `search.tsx` | Stage 9A natural-language search through `POST /search`. The page keeps the complete interpretation response in hook/page state and always renders “Bielik zinterpretował…” with topic, active filters, warnings, fallback and clarification. Manual filter editing/shareable explicit URLs are stage 9B. |
...
| `useSearch` | Natural-language search + interpretation state | `POST /search` with `{natural_query, limit}`; response `interpretation`, `fallback_used`, `results`, `search_id` stay together in `SearchResponse` |
