...
Terraform configuration provisioning the OpenVPN relay VM from [ADR-023](../../../docs/adr/adr-023-gcloud-vpn-relay-nas-access.md). **Phase 1 + partial Phase 2, deployed and verified end-to-end 2026-08-26** (PR #584): the VPN server + firewall rule, a delegated Cloud DNS zone that keeps the relay's hostname current across restarts, and a start/stop/status Cloud Function. Still not implemented: a public HTTP endpoint for that function (API Gateway equivalent) and the Firebase Hosting frontend from the ADR — deliberately deferred, see "DNS + start/stop" below.
...
**Second bug, fixed in the script but not (and never will be, on this particular instance) applied via Terraform:** `openvpn-install.sh install --endpoint` was originally passed `$EXTERNAL_IP`, not the hostname — that value gets baked verbatim into `/etc/openvpn/server/client-template.txt` and reused for every client `.ovpn` generated from then on (including future ones added by hand), so it would have silently defeated the whole point of this DNS work: every profile would still need regenerating on every restart. Fixed in the script to use `vpn.gcloud.lenie-ai.eu` instead. **This fix is not live via Terraform state on the running instance** — applying it via `terraform apply` would force another full VM replace (see below), so instead the running server's `client-template.txt` was patched by hand (`sed -i 's/^remote .*/remote vpn.gcloud.lenie-ai.eu 1194/'`) and the already-downloaded `nas.ovpn`/`demo-laptop.ovpn` were fixed the same way before handing them to the operator. The script fix in this repo will only take effect automatically the next time this instance is genuinely replaced for some other reason (image update, machine type change, etc.) — if you `terraform plan` and see `google_compute_instance.vpn_relay must be replaced` with `metadata_startup_script` as the diff, that's this pending, deliberately-deferred change, not new drift. **The hand-patched hostname endpoint was confirmed working live 2026-08-26**: stop → start cycled the VM to a new IP (`34.118.10.127` → `34.158.231.226`), Cloud DNS updated automatically, the operator reconnected using the *same* `nas.ovpn` (hostname-based, no re-download needed) without any manual IP lookup — this is the actual end-to-end proof the DNS delegation work was for.

**Any `service_account` block change forces a full instance replace** (new boot disk, new OpenVPN CA/certs, every existing `.ovpn` profile invalidated) — GCE doesn't allow swapping a running instance's service account via update, only via recreate. Confirmed live 2026-08-26 when `vpn_relay_vm`'s service account was first attached. Apparently **any** `metadata_startup_script` change forces the same replace in this provider version too (not just service_account changes) — plan before assuming a script edit is a safe in-place update.
...
**Re-confirmed with the hostname-based setup** (2026-08-26): started via `vpn-relay-control` (Cloud Function), reconnected NAS's QVPN profile (now pointing at `vpn.gcloud.lenie-ai.eu`, not a hardcoded IP), verified the tunnel from both ends — `tun2001` interface up on the NAS (`ip addr` over SSH as `admin@192.168.200.7`) and `curl http://192.168.200.7:5055/` from the relay returned `HTTP 200`. Stopped again afterward via the same Cloud Function.
