...
Currently a single purpose: the OpenVPN relay from [ADR-023](../../docs/adr/adr-023-gcloud-vpn-relay-nas-access.md), which lets remote clients (Chrome extension, laptop away from home) reach the NAS (`192.168.200.7`) despite the home ISP's CGNAT. See [`terraform/CLAUDE.md`](terraform/CLAUDE.md) for what's actually deployed — **Phase 1 + partial Phase 2, verified end-to-end 2026-08-26**: VM + OpenVPN server, a delegated `gcloud.lenie-ai.eu` Cloud DNS zone that tracks the relay's ephemeral IP across restarts, and a start/stop/status Cloud Function (no public HTTP endpoint / frontend yet).
