...
| Script | Pipeline |
|--------|----------|
| `documents_pipeline.py` | YouTube processing â†’ webpage download (S3/live) â†’ markdown cleanup â†’ embedding |
| `document_md_decode.py` | Markdown decoding â†’ link extraction â†’ correction â†’ embedding |
