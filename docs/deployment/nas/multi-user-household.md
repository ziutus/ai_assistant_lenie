...
## 3. Operator i zarządzanie jobami

Klucz `service` pozostaje kontem technicznym bez tożsamości człowieka i z pełnym
dostępem do akcji operacyjnych. Klucz `user` może obecnie ręcznie utworzyć tylko
tymczasowy job `legacy_aws_pull`; API zapisuje jego `user_id` w
`jobs.initiated_by_user_id`. Dzięki temu właściciel instancji może uruchomić
bridge z panelu bez umieszczania klucza service w przeglądarce, a historia joba
odpowiada na pytanie, kto rozpoczął import.

To jest celowo wąskie, przejściowe uprawnienie. Retry i cancel jobów oraz
tworzenie innych typów jobów pozostają service-only, ponieważ mogą wpływać na
pracę innych użytkowników albo uruchamiać kosztowne procesy.

Przed udostępnieniem instancji większej liczbie osób potrzebna jest formalna
rola **operatora** (najlepiej przypisana do użytkownika lub klucza, z centralną
kontrolą uprawnień). Operator powinien móc zarządzać dozwolonymi jobami i być
zapisywany jako ich inicjator; zwykły użytkownik nie powinien automatycznie
dziedziczyć tych uprawnień. Obecne jednoużytkownikowe wdrożenie traktuje
zaufany klucz użytkownika jako operatora wyłącznie dla `legacy_aws_pull`.

## 4. Czego brakuje i warto dodać teraz (tanie, bez kolejek/Redis)
...
## 5. Wymagania funkcjonalne: goście read-only
...
## 6. Co świadomie NIE jest robione teraz
...
## 7. Granica: kiedy household przestaje wystarczać
...
Dopóki żaden z tych sygnałów się nie pojawił, sekcje 4 i 5 powyżej to całość potrzebnej specyfikacji.
...
5. *(świadomie odłożone — dopiero projektowanie, nie implementacja)* Rola read-only: trzeci poziom w `AuthContext`, centralna lista dozwolonych endpointów per rola, tabela audytu żądań dla tej roli — patrz sekcja 5.
