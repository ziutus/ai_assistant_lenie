...
|---|---|---|---|
| Inwarianty bezpieczeństwa aplikacji | [Inwarianty z profilu lokalnego](local-development-security.md#inwarianty-bezpieczeństwa-aplikacji) nadal egzekwowane pod nową ekspozycją: uprawnienia metod, ochrona SSRF ruchu wychodzącego, brak skutków `OPTIONS`, ograniczony cache uwierzytelniania. | Testy regresyjne uruchamiane w CI: odmowa metod mutujących dla `read_only`, redirect do celu wewnętrznego zablokowany, `OPTIONS` bez operacji storage, cache pod obciążeniem unikalnych kluczy pozostaje ograniczony. | Zespół aplikacji. |
