...
- **Chunk analysis models** (backing `chunk_review_routes.py` / `document_analysis_service.py`):
  ...
  - `DocumentChunk` — one row per chunk of a run. `type`: `TEMAT` (on-topic) / `REKLAMA` (sponsored) / `SZUM` (extraction junk — portal nav, cookie banners; auto-approved, never sent to an LLM for note-writing). `status`: `pending` → `approved` (or `needs_reanalysis`/`split_requested`/`split`/`skipped`). `obsidian_note_paths` — array of vault-relative paths, populated by the `/lenie-obsidian-note` skill when a note is written from the chunk. `obsidian_note_not_needed` — reviewer flag "this TEMAT chunk is not worth a standalone note" (thin / uninteresting); the chunk stays in every other pipeline (still embedded when `approved`) but drops out of the "chunks still missing an Obsidian note" counter/filter. Set per-chunk via `PATCH /chunk/<id>` or in bulk via `POST /analysis_run/<id>/mark_notes_not_needed` (`{"value": bool}`, default true — flags only note-less chunks; `false` clears them). `document_repository._missing_obsidian_note_chunk_conditions()` also treats a **one-chunk document that already has a document-level `Document.obsidian_note_paths`** as covered (the document note *is* that chunk's note), computed at read time — no chunk-level write, works retroactively.
...
- **`document_repository.py`** — `DocumentRepository`: query layer using SQLAlchemy session. Requires `session` parameter. Methods: `get_list()` (paginated/filtered; each row also carries `chunks_missing/with_obsidian_notes`, `groups`, and `link_count`/`proposed_link_count` — confirmed/proposed `document_links` across both endpoints, batched via `_count_document_links()`), `get_similar()` (pgvector cosine search), `search_text()` (lexical `ILIKE` candidate search, diacritic-folded via `unaccent()`), `list_by_filters()` (filter-only listing, no text/embedding query — stage 6 session B; empty `filters` legally lists everything newest-first, `sort` maps `SearchSort` to `published_on`/`created_at` ordering, `RELEVANCE` falls back to `INGESTED_DESC`'s ordering since there's no query to score against), `get_next_to_correct()`, `get_count()`, `embedding_add()`, `embedding_delete()`. `get_similar()`, `search_text()` and `list_by_filters()` all accept an optional `filters: SearchFilters` (stage 6) applied via `search.sql_filters.build_document_filters()` — the same builder for all three, so lexical, vector and filter-only search share identical constraints (stage 11c removed the legacy single-value `project` kwarg — collection filtering goes through `filters.collection_name`, resolved against the `collections` lookup table).
...
- **`ceidg_client.py`** — `get_company_by_nip(nip)` (official CEIDG data warehouse API v3, `https://dane.biznes.gov.pl/api/ceidg/v3/firma?nip=...`, `CEIDG_API_KEY` JWT from config/Vault — free registration via Profil Zaufany/mObywatel at biznes.gov.pl; `None` on missing/invalid key, no match (`204`), or request failure), `company_to_organization_fields(firma)` (maps the raw response onto `ContactOrganization` fields: name/address/correspondence address/status→`is_current`+`suspended_at`/`end_date`/start date/`website`, notes with tel.+e-mail — only includes keys actually present, callers merge rather than overwrite). Deliberately **not** a scrape of the public CEIDG search UI (`aplikacja.ceidg.gov.pl/CEIDG/CEIDG.Public.UI/SearchDetails.aspx`) — that's an old ASP.NET WebForms page Lenie's webpage downloader can't parse into usable text (confirmed failure, `ERROR_DOWNLOAD`). Wired into `POST /contacts/<id>/organizations/ceidg_lookup` (`contact_routes.py`) — creates or refreshes a contact's `jdg` organization entry from a live CEIDG match, falling back to an existing entry's stored NIP when none is given in the request (so the endpoint doubles as a periodic "refresh from CEIDG" action); merges a previously-known phone number back into `notes` when CEIDG's response omits `telefon` (common — many owners opt out of publishing it) rather than silently dropping it. CEIDG only covers sole proprietorships (JDG) — a spółka needs KRS instead, not handled here (no KRS client). Which register an organization is in is stored explicitly in `contact_organizations.registry` (`ceidg` / `krs` / `other`, NULL = unknown), independent of `org_type` (the *person's* relation: an `employment` employer can be a JDG or a company): the contact card offers the CEIDG and/or KRS lookup link based on it (both when NULL), `POST .../organizations` defaults a `jdg` row to `ceidg`, and the CEIDG lookup sets it on every row it creates or refreshes.
- **`place_verification.py`** — `verify_document_places(session, doc, text)`: the document's `geogName`/`placeName` entities → geocoder via `geocode_cache` (one live call ever per distinct name, negative results cached) → `article_tagging.confirm_places_with_llm()` picks the places actually discussed → `miejsce-<slug>` tags merged into `doc.tags`. Tags are slugged from the geocoder's canonical spelling (`canonical_place_name`), not the NER surface form — spaCy doesn't always lemmatize proper names, so "Kijów"/"Kijowa" used to yield duplicate tags; entities sharing a canonical name merge their mention counts before the auto-confirm threshold and LLM check. Countries are skipped (own `kraj-*` pipeline). Called from `create_run()` step 11d and `POST /website_entities`. One-off cleanup of pre-existing duplicate tags: [`imports/fix_place_tags.py`](../imports/CLAUDE.md).
