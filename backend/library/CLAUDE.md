...
├── search/           # Typed search domain models + parser (stages 1-4 of docs/search-rebuild-implementation-plan.md)
...
- **`stalker_web_documents_db_postgresql.py`** — `WebsitesDBPostgreSQL`: query layer using SQLAlchemy session. Requires `session` parameter. Methods: `get_list()` (paginated/filtered), `get_similar()` (pgvector cosine search), `search_text()` (lexical `ILIKE` candidate search, diacritic-folded via `unaccent()`), `get_next_to_correct()`, `get_count()`, `embedding_add()`, `embedding_delete()`.
- **`search_service.py`** — `SearchService`: hybrid search orchestrator for `GET /search`. `search_similar()` combines `search_text()` (lexical) and `get_similar()` (semantic/pgvector) candidates, then `_merge_results()` scores and de-duplicates them using its own Python-side diacritic folding (`_normalise()`), which must stay behaviorally aligned with the SQL-side `unaccent()` folding in `search_text()`. An optional `period_from`/`period_to` year window (BCE = negative years) keeps only documents whose classified `document_time_periods` range overlaps it — documents with no classified period drop out when the filter is set. See [`docs/search-hybrid.md`](../../docs/search-hybrid.md) for why plain `ILIKE` isn't enough and why this two-layer folding exists.
