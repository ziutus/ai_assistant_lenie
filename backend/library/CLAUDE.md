...
- **SQLAlchemy ORM**: Database access uses SQLAlchemy ORM (`db/models.py`, `db/engine.py`). `Document` model handles single-document CRUD; `WebsitesDBPostgreSQL` handles queries and vector search. Connection supports optional SSL via `POSTGRESQL_SSLMODE` env var (required for AWS RDS).
