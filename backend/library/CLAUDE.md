...
│   ├── sql_filters.py  # the ONE shared filter builder (stages 6-7): publisher via publisher_id subqueries; author via role='author' canonical/alias EXISTS plus byline fallback only for unnormalized docs; discovery source via physical sources/source. Never joins information_sources.
