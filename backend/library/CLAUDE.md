...
- **`entity_service.py`** — `refresh_document_entities(session, doc_id, text)` (NER → aggregate → drop entities matched by an `ner_exclusions` rule (`is_excluded()`: case-insensitive text, `entity_type='*'` wildcard, scope `global`/`author`) → replace the document's `document_entities` rows; empty extraction leaves existing rows untouched), `get_document_entities(session, doc_id)` (grouped by type, most-mentioned first, each item carries its row `id`; place entities carry `verified`/`lat`/`lon`/`display_name` from their `geocode_cache` link; resolved persons carry `link_id`). Called from `document_analysis_service.create_run()` (step 11c, whole-document runs only), `article_browser.py` (`[w]`/`[e]` actions) and the `/website_entities` endpoints in `server.py`.
