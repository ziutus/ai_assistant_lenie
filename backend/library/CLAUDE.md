...
│   └── sql_filters.py  # build_document_filters(filters: SearchFilters) → list[ColumnElement[bool]] (stage 6) — the ONE filter builder search_text(), get_similar() AND list_by_filters() apply via `.where(*conditions)` before LIMIT (stalker_web_documents_db_postgresql.py), so lexical, vector and filter-only search use identical constraints. Maps collection_name→web_documents.project (exact match — today it's a flat string, not a lookup table), published_on_*→date_from, ingested_at_*→created_at, document_types/languages→IN clauses, subject_period_*→a correlated EXISTS subquery against document_time_periods (open-ended bound on either side never disqualifies a label-only period). author_name/publisher_name/publisher_domain/discovery_source_name raise NotImplementedError — they need name resolution (stage 7), not yet built; a silently-ignored filter would be worse than a loud one. SearchService.search_similar() (the ONLY live search path, backing `/website_similar`) now builds a SearchFilters internally and passes it to both repo calls — the period-year window used to be a Python-side post-filter applied to an already-LIMITed candidate list (could hide genuine matches beyond the first N); it's SQL now, before LIMIT. `SearchService.search_by_filters()` (session B) never generates an embedding.
...
- **`stalker_web_documents_db_postgresql.py`** — `WebsitesDBPostgreSQL`: query layer using SQLAlchemy session. Requires `session` parameter. Methods: `get_list()` (paginated/filtered), `get_similar()` (pgvector cosine search), `search_text()` (lexical `ILIKE` candidate search, diacritic-folded via `unaccent()`), `list_by_filters()` (filter-only listing, no text/embedding query — stage 6 session B; empty `filters` legally lists everything newest-first, `sort` maps `SearchSort` to `date_from`/`created_at` ordering, `RELEVANCE` falls back to `INGESTED_DESC`'s ordering since there's no query to score against), `get_next_to_correct()`, `get_count()`, `embedding_add()`, `embedding_delete()`. `get_similar()`, `search_text()` and `list_by_filters()` all accept an optional `filters: SearchFilters` (stage 6) applied via `search.sql_filters.build_document_filters()` — the same builder for all three, so lexical, vector and filter-only search share identical constraints; `get_similar()`/`search_text()` also keep the older single-value `project` kwarg (both apply, AND'd, if both are given).
- **`search_service.py`** — `SearchService`: hybrid search orchestrator for `/website_similar` (the only live search endpoint pre-stage-8). `search_similar()` combines `search_text()` (lexical) and `get_similar()` (semantic/pgvector) candidates, then `_merge_results()` scores and de-duplicates them using its own Python-side diacritic folding (`_normalise()`), which must stay behaviorally aligned with the SQL-side `unaccent()` folding in `search_text()`. An optional `period_from`/`period_to` year window (BCE = negative years) is built into a `SearchFilters` and passed to both repo calls — applied as a correlated `EXISTS` against `document_time_periods` in SQL, before `LIMIT` (stage 6; previously a Python-side post-filter on an already-`LIMIT`ed candidate list, which could hide genuine matches beyond the first N). A reversed `period_from`/`period_to` is swapped via `normalize_year_range()`, not rejected; an out-of-domain year or a blank `project` string degrades to no filter rather than raising `SearchQueryValidationError` — this method has always accepted untrusted HTTP params and must keep degrading gracefully. `search_by_filters()` (stage 6 session B) is a thin delegate to `list_by_filters()` for "find documents matching these criteria" with no free-text phrase — it never calls `embedding.get_embedding()`, avoiding wasted latency/cost for a query with no text to embed. See [`docs/search-hybrid.md`](../../docs/search-hybrid.md) for why plain `ILIKE` isn't enough and why the two-layer folding exists.
