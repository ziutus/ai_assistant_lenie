...
class ContactLookupResult(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lookup_type IN ('phone', 'linkedin', 'web', 'email', 'facebook')",
            name="ck_contact_lookup_results_lookup_type",
        ),
        CheckConstraint(
...
class Address(Base):
    ...
    block_number: Mapped[str | None] = mapped_column(String(20))
...
class ContactOrganization(Base):
    ...
    start_date: Mapped[datetime.date | None] = mapped_column(Date)
    end_date: Mapped[datetime.date | None] = mapped_column(Date)
    suspended_at: Mapped[datetime.date | None] = mapped_column(Date)
    verified_at: Mapped[datetime.date | None] = mapped_column(Date)
    status: Mapped[str] = mapped_column(String(20), nullable=False, server_default="confirmed")
    source_url: Mapped[str | None] = mapped_column(Text)
    notes: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=False, server_default=func.now())
