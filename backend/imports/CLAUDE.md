...
├── whatsapp_chat_import.py   # Full message log + attachment import into chat_conversations/chat_messages (raw archive, not the LLM-distilled Contact profile)
├── whatsapp_chat_backfill_contacts.py  # One-off: re-resolve contact_id for already-imported chat_messages rows missed before sender-suffix stripping existed
...
### Full WhatsApp chat log import (`chat_conversations`/`chat_messages` tables)

- **`whatsapp_chat_import.py`** — imports the raw message-for-message log of a WhatsApp export (every participant, not just the account owner) plus attachments, into `chat_conversations`/`chat_messages`. Deliberately separate from `whatsapp_neighbor_profiles.py` above: that script reads the same export format but writes only LLM-distilled facts onto `Contact.whatsapp_profile` and never stores raw text; this one stores the full archive and never calls an LLM. Both can run against the same export independently. Accepts either WhatsApp's media-included export (a `.zip` containing one `.txt` plus every attachment, auto-detected by file signature) or a text-only `.txt` (no attachment bytes available, only filenames get recorded). `--chat-key` is a stable id chosen once per conversation (kept the same across repeat exports of the same group — WhatsApp reuses a display name across "Czat ogólny", "Czat ogólny (1)", ...); re-running against a newer export of the same chat only inserts messages not already present, both via a `last_imported_at` watermark (skip-ahead optimization) and a `dedup_hash` unique constraint (WhatsApp exports carry no stable per-message id, so identity is `sha256(chat_key|sender_name_raw|sent_at|content-or-attachment-filename)`) — safe to re-run on every fresh export. `contact_id` is resolve-only (`library.whatsapp_parser.find_contact` — phone then normalized name against the existing contact book) and never creates a new `Contact`, unlike `whatsapp_neighbor_profiles.py`, since a full multi-participant log seeing an unfamiliar name isn't a reason to add them to the private contact book. `--sender-suffix` (default "Tuwima Gardens", same convention `google_contacts_import.py` strips) is cut off the sender's display name (`library.whatsapp_parser.strip_sender_suffix`) *before* that name match — WhatsApp's own export carries this trailing " - <suffix>" tag but the contact book never does, so without stripping it the normalized-name key never matches (found 2026-09-21: ~130 senders on the "Tuwima Gardens" chat had a real, unambiguous matching contact but `contact_id` stayed `NULL`; backfilled for already-imported rows via `whatsapp_chat_backfill_contacts.py` below). Attachments are uploaded via `library/storage.py`'s `ObjectStorage` (same abstraction as `document_images`/`contact_photos`) under `chat_media/<chat_key>/<sha256-of-bytes>_<original filename>`; some attachments are excluded from the export by WhatsApp itself (its own per-file/total export size limit) and show up as a bare "media pominięte" line with only a filename, no bytes — recorded with `media_original_filename` set but `media_storage_key` left `NULL`. **Privacy**: stores every participant's actual message text, so only point it at chats the account owner has the right to archive in full (e.g. a group they're a member of). Parsing/attachment-detection/sender-resolution helpers are shared with `whatsapp_neighbor_profiles.py` via `library/whatsapp_parser.py` — a parser fix only needs to happen once.
  ```
  cd backend
  python imports/whatsapp_chat_import.py --export "chat.zip" --chat-key "tuwima-gardens/czat-ogolny" --chat-name "Tuwima Gardens - Czat ogólny"   # dry-run: stats only
  python imports/whatsapp_chat_import.py --export "chat.zip" --chat-key "..." --chat-name "..." --apply
  python imports/whatsapp_chat_import.py --export "chat.zip" --chat-key "..." --chat-name "..." --apply --skip-media   # text log only, no MinIO uploads
...
- **`whatsapp_chat_backfill_contacts.py`** — one-off: re-resolves `contact_id` for already-imported `chat_messages` rows left `NULL` before `whatsapp_chat_import.py` stripped the sender's community-name suffix — re-running the importer itself won't fix these, since `dedup_hash` for an already-present message is unchanged and the row is skipped as a duplicate rather than updated. Only touches rows with `contact_id IS NULL`; uses the same resolve-only `find_contact()` (ambiguous same-name contacts stay unresolved, matching the importer's own safety rule).
  ```
  cd backend
  python imports/whatsapp_chat_backfill_contacts.py --chat-key "tuwima-gardens/czat-ogolny"          # dry-run
  python imports/whatsapp_chat_backfill_contacts.py --chat-key "tuwima-gardens/czat-ogolny" --apply
