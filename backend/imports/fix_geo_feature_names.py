...
seas/straits/gulfs/canals (library/geo_feature_gazetteer.py).

Before this gazetteer existed, a multiword geo feature mentioned only in
inflected cases ("Morza Czerwonego", "Zatoki Perskiej") had nothing to
recover the nominative form from — ner_client.py's nominative-surface
preference only helps when at least one in-text mention is actually in the
nominative case. entity_text ended up stored inflected (or with spaCy's
gender-mangled lemma), which then also broke geocoding (the geocoder got the
inflected string). ner_client.py now avoids this for every future NER run;
this script fixes the document_entities rows that were written before that
fix shipped.
...
Usage:
    cd backend
    .venv/Scripts/python imports/fix_geo_feature_names.py            # dry-run (default)
    .venv/Scripts/python imports/fix_geo_feature_names.py --apply
    .venv/Scripts/python imports/fix_geo_feature_names.py --id 9394  # single document
"""
...
from library.geo_feature_gazetteer import canonical_geo_feature_name  # noqa: E402
...
def plan_document(rows: list[DocumentEntity]) -> list[tuple[DocumentEntity, list[DocumentEntity], str]]:
    """Group a document's place rows by (entity_type, canonical name).
    ...
    for row in rows:
        canonical = canonical_geo_feature_name(row.entity_text)
...
def main():
    parser = argparse.ArgumentParser(description="Canonicalize known geo-feature entity_text values.")
