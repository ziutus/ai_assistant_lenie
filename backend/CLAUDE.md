...
| **Entities (NER)** | `/website_entities` (GET: stored persons/places; POST: re-run NER + verify places + resolve persons), `/persons` (GET: fuzzy registry search), `/person_documents` (GET: all documents mentioning a person) — see `library/entity_service.py`, `library/person_registry.py` |
