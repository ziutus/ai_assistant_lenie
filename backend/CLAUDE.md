...
| **Reader** | GET `/document/<id>/chapter/<pos>` (one chapter's text for `/read/:id` — title, footnotes, and `images`: book-PDF illustrations, presigned MinIO URLs via `storage.presigned_get_url()`, each with an `inline` flag saying whether it has a `[imgN]` marker in the text or belongs in the reader's chapter-end "Ilustracje" section), GET `/document/<id>/images` (full storage-backed image list for a document, same item shape minus `inline` — diagnostic/editor use), GET `/document/<id>/anchor/<anchor_id>` (resolves an in-text `[#anchor_id]` marker — e.g. a book table's real occurrence, see `library/book_pdf_import.py`'s `_link_table_captions()` — to the reader chapter `position` it currently falls in, computed fresh via `detect_chapters()` rather than trusting a number baked in at import time) — see `library/chunk_review_routes.py`, `library/document_images.py`. GET `/document/<id>/readiness` (reader "Stan przetwarzania": per-stage done/partial/todo/na checklist — content, chunks (run + review closed), embeddings, NER, enrichment, quality, thematic tags, plus an informational Obsidian-note row — with an overall `ready`/`needs_work` verdict; which steps are REQUIRED depends on `document_type` via `_READINESS_REQUIRED_BY_TYPE`; read-only, every signal is an existing column/row) |
| **Timeline** | GET `/document/<id>/events` (stored dated events discussed in the document, sorted chronologically; extraction: `imports/extract_events.py`), GET `/document/<id>/time_periods` (historical periods the content is about, per reader chapter for books; classification: `imports/extract_time_periods.py`), GET `/document/<id>/tones` (emotional tone + language register per chapter; classification: `imports/extract_tones.py`), GET `/document/<id>/control_questions` (geopolitical control questions the document answers, per chapter) / POST `/document/<id>/select_control_questions` (on-demand trigger of the cheap-LLM router when enrichment hasn't run yet, optional `chapter_position`/`model` body fields; cheap-LLM router: `library/control_question_selection.py`, CLI equivalent: `imports/select_control_questions.py`) |
...
| **Document links** | GET `/document/<id>/relationship_graph` (server-assembled provenance + organization graph; also emits `document:<id>` nodes for confirmed document-to-document links), GET/POST `/document/<id>/links` (typed directed links to other library documents, each resolved from this document's perspective — `direction`/`label`/`other_document`), POST `/document/<id>/links/detect` (propose `references` links for other documents whose URL this one's text mentions), PATCH `/document_links/<id>` (`status`: confirmed/rejected), DELETE `/document_links/<id>` — see `library/document_links_routes.py`, `library/document_links_service.py` |
| **Topics** | GET/POST `/topics`, GET/PATCH `/topics/<id>`, POST `/topics/<id>/items`, DELETE `/topic_items/<id>` — generic topics linking documents, contacts, conversations and messages; grouped detail, archive and paginated list — `library/topic_routes.py`, `library/topic_service.py` |
| **Auth & identity** | `/whoami`, `/api_keys` (GET/POST), `/api_keys/<id>` (DELETE) — see `library/api_key_routes.py` |
| **Chat logs** | GET `/chat_conversations` (paginated list of imported chats — currently WhatsApp only, `imports/whatsapp_chat_import.py`), GET `/chat_conversations/<id>/messages` (paginated message thread, chronological by default, optional `?sender=`/`?contact_id=`/`?message_type=`/`?sort=desc`; attachments served as presigned MinIO URLs via `storage.presigned_get_url()`) — read-only, see `library/chat_routes.py`. `GET /contacts/<id>` (private contact book) also embeds a `chat_conversations` summary (conversation id/name/platform/message_count/last_message_at) for every chat the contact has a resolved `chat_messages.contact_id` in — `library/contact_routes.py`'s `_contact_chat_conversations()` |
...
Ad-hoc single-item tools and bulk import scripts live in [`imports/`](imports/CLAUDE.md): `youtube_add.py`, `youtube_batch_analyze.py`, `dynamodb_sync.py`, `freedom_house_import.py`, `control_questions.py`, `migrate_data_to_cache.py`. Feed monitoring runs in `library/feed_monitor_service.py` and `worker.py`.
...
| `imports/` | [imports/CLAUDE.md](imports/CLAUDE.md) | CLI tools & import scripts; feed monitoring is handled by the REST API and NAS worker. |
