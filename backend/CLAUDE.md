...
| **Timeline** | GET `/document/<id>/events` (stored dated events discussed in the document, sorted chronologically; extraction: `imports/extract_events.py`), GET `/document/<id>/time_periods` (historical periods the content is about, per reader chapter for books; classification: `imports/extract_time_periods.py`), GET `/document/<id>/tones` (emotional tone + language register per chapter; classification: `imports/extract_tones.py`), GET `/document/<id>/control_questions` (geopolitical control questions the document answers, per chapter; cheap-LLM router: `library/control_question_selection.py`, CLI: `imports/select_control_questions.py`) |
