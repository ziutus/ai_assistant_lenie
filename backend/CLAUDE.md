...
| **Chat logs** | GET `/chat_conversations` (paginated list of imported chats — currently WhatsApp only, `imports/whatsapp_chat_import.py`), GET `/chat_conversations/<id>/messages` (paginated message thread, chronological by default, optional `?sender=`/`?message_type=`/`?sort=desc`; attachments served as presigned MinIO URLs via `storage.presigned_get_url()`) — read-only, see `library/chat_routes.py` |
