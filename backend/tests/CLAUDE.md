...
| `test_db_models.py` | `Document` columns (30), STI subclasses (8), `dict()` output (33 keys), lookup models |
...
| `test_feed_parser.py` | Pure feed parser helpers and filtering |
...
- **Modules under test with heavy module-level imports** (`document_md_decode`): tests install minimal stubs in `sys.modules` only where needed, then remove them so the `importorskip` pattern in other modules keeps working.
